...
const URI_ATTRIBUTES = new Set(['href', 'src', 'action', 'formaction', 'xlink:href', 'data']);
...
function decodeEntities(value: string): string {
  return value.replace(/&#x([0-9a-fA-F]+);/g, (_, hex) => String.fromCharCode(parseInt(hex, 16)))
    ...
    .replace(/&[a-zA-Z]+;/g, (match) => ENTITY_MAP[match.toLowerCase()] ?? match);
}

export class SecurityFilter {
  private options: Required<Pick<SanitizerOptions, 'allowedTags' | 'allowedAttributes' | 'allowedSchemes' | 'maxAttributeLength'>>;
  ...
  constructor(options: SanitizerOptions) {
    this.options = {
      allowedTags: options.allowedTags ?? [
        ...
        'img', 'hr', 'sup', 'sub', 'dl', 'dt', 'dd', 'figure', 'figcaption',
      ],
    ...
    this.allowedTagSet = new Set(this.options.allowedTags.map(t => t.toLowerCase()));
  }
  ...
  private isSchemeAllowed(value: string): boolean {
    const decoded = decodeEntities(value).replace(/[\x00-\x20]+/g, '').trim();
