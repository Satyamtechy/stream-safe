...
const xssVectors = [
  ...
  { input: '<math><mtext></mtext><mglyph><svg><mtext><style><img src=x onerror=alert(1)></style></mtext></svg></mglyph></math>', expected: '' },
  { input: '<object data="javascript:alert(1)">', expected: '' },
  { input: '<embed src="javascript:alert(1)">', expected: '' },
  { input: '<form action="javascript:alert(1)"><input type=submit>', expected: '' },
  { input: '<button formaction="javascript:alert(1)">click</button>', expected: '' },
  { input: '<a href="data:text/html,<script>alert(1)</script>">x</a>', expected: '<a>x</a>' },
  { input: '<img src="x" longdesc="javascript:alert(1)">', expected: '<img src="x" />' },
  { input: '<div onclick=alert(1)>text</div>', expected: 'text' },
  { input: '<p onmouseover=alert(1)>hover</p>', expected: '<p>hover</p>' },
  { input: '<img src=1 onerror="alert(1)" alt="test">', expected: '<img src="1" alt="test" />' },
