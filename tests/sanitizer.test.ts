...
describe("createStreamSanitizer", () => {
  ...
  it("allows safe tags from preset", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<p>Hello <strong>world</strong></p>") +
      sanitizer.flush();
  ...
  it("strips script tags entirely", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<script>alert(1)</script>") + sanitizer.flush();
  ...
  it("strips event handler attributes", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write('<p onclick="alert(1)">text</p>') + sanitizer.flush();
  ...
  it("strips javascript: URLs", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write('<a href="javascript:alert(1)">click</a>') +
      sanitizer.flush();
  ...
  it("allows safe href URLs", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write('<a href="https://example.com">link</a>') +
      sanitizer.flush();
  ...
  it("closes unclosed tags on flush", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<p>Hello <strong>world") + sanitizer.flush();
  ...
  it("handles self-closing tags like br and img", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write('<br><img src="x.png" />') + sanitizer.flush();
  ...
  it("handles tag split across chunks: <str + ong>", () => {
    ...
    const result =
      sanitizer.write("<str") +
      sanitizer.write("ong>text</strong>") +
      sanitizer.flush();
    expect(result).toContain("<strong>");
    expect(result).toContain("text");
  });
  ...
  it("handles attribute split across chunks", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<a hre") +
      sanitizer.write('f="https://x.com">link</a>') +
      sanitizer.flush();
  ...
  it("handles script tag split: <scr + ipt>", () => {
    ...
    const result =
      sanitizer.write("<scr") +
      sanitizer.write("ipt>alert(1)</script>") +
      sanitizer.flush();
    expect(result).not.toContain("<script");
    expect(result).not.toContain("alert");
  });
  ...
  it("handles closing tag split: </str + ong>", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<strong>text</str") +
      sanitizer.write("ong>") +
      sanitizer.flush();
    expect(result).toContain("<strong>");
    expect(result).toContain("text");
  ...
  it("handles chunk that is just <", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<") + sanitizer.write("p>text</p>") + sanitizer.flush();
    expect(result).toContain("<p>");
    expect(result).toContain("text");
  });
  ...
  it("blocks <img onerror=alert(1)>", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<img onerror=alert(1)>") + sanitizer.flush();
  ...
  it('blocks <a href="javascript:alert(1)">', () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write('<a href="javascript:alert(1)">x</a>') +
      sanitizer.flush();
    expect(result).not.toContain("javascript:");
  });
  ...
  it('blocks <iframe src="evil.com">', () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write('<iframe src="evil.com"></iframe>') + sanitizer.flush();
  ...
  it('blocks <div onmouseover="alert(1)">', () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write('<div onmouseover="alert(1)">text</div>') +
      sanitizer.flush();
  ...
  it("blocks <style>body{background:url(javascript:)}</style>", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<style>body{background:url(javascript:)}</style>") +
      sanitizer.flush();
  ...
  it("blocks nested script in allowed tag", () => {
    const sanitizer = createStreamSanitizer(presets.llmChat);
    const result =
      sanitizer.write("<p><script>alert(1)</script></p>") + sanitizer.flush();
    expect(result).not.toContain("<script");
    expect(result).not.toContain("alert");
    expect(result).toContain("<p>");
  });
  ...
  it("strips oversized attributes", () => {
    ...
    const result =
      sanitizer.write(`<a href="${longAttr}">link</a>`) + sanitizer.flush();
